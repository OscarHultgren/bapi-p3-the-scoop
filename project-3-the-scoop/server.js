...
//Functions written for assignment starts here
function createComment(url, request) {
  const requestComment = request.body && request.body.comment;
  const response = {};
//If all required information is supplied create a new comment. Else return 400
...
//Set the response to be returned to the comment and status 201
    response.body = {comment: comment};
    response.status = 201;
  } else {//Return status 400 if something is missing
   response.status = 400;
  }
...
function updateComment(url, request) {
  //Find id of the comment from request and declare constants if they exist
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const savedComment = database.comments[id];
  const requestComment = request.body && request.body.comment;
  const response = {};

  //Return 400 for bad request and 404 if comment is not found
  if (!id || !requestComment) {
    response.status = 400;
  } else if (!savedComment) {
    response.status = 404;
  } else {
    //If no comment is sent in the request it will be set to the same as before
...
function deleteComment(url, request) {
  //Find out which comment to delete and save it to a constant
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const savedComment = database.comments[id];
  const response = {};
  if (savedComment) {
    database.comments[id] = null;//Delete the comment form the database
    const userCommentIds = database.users[savedComment.username].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);//Remove the comment from
    ...
    response.status = 404;//No comment found
  }
...
function loadDatabase() {
  try {
    const db = yaml.safeLoad(fs.readFileSync('database.yml', 'utf8'));
    return db;
  } catch (e) {
    console.log(e);
  }
...
function saveDatabase() {
  ...
  fs.writeFile("database.yml", db, function(err) {
      if(err) {
